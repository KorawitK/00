...
#define Motor1_IN1 23  //23
#define Motor1_IN2 13  //13
...
void loop() {
  ...
  if (Run == true) {
    ...
    if (PS4.L1() == 1 && stateBall_Arm == 0) {
      ServoArm_ball();
    } else if (PS4.L1() == 1 && stateBall_Arm == 1) {
      ServoArm_ball();
    }
...
void Servo12() {
  if (state_S12 == 1) {
    for (int pulse = 410; pulse > 300; pulse -= 1) {
      pwm.setPWM(servo1, 0, pulse);
      pwm.setPWM(servo2, 0, pulse);
    }
  }
  if (state_S12 == 0) {
    for (int pulse = 300 ; pulse < 410; pulse += 1) {
      pwm.setPWM(servo1, 0, pulse);
      pwm.setPWM(servo2, 0, pulse);
    }
  }
...
void Servo34(int degree) {
  if (state_S34 == 1) {
    for (int pulse = 335; pulse > 240; pulse -= 1) {
      pwm.setPWM(servo3, 0, pulse);
      pwm.setPWM(servo4, 0, pulse);
    }
  }
  if (state_S34 == 0) {
    for (int pulse = 240; pulse < 335; pulse += 1) {
      pwm.setPWM(servo3, 0, pulse);
      pwm.setPWM(servo4, 0, pulse);
...
void StepperY() {
    int Limit_Y = digitalRead(LmY);
  if (stateY == 1 && Ty != 1400 && Limit_Y == HIGH) {
    enter3 = true;
    while (Ty < 1400) {
      if (enter3 == true) {
        digitalWrite(dirPinY, HIGH);
...
void motion() {
  ...
  if (PS4.LStickY() >= 40) {
    ...
    analogWrite(Motor1_IN1, turn_left);
    analogWrite(Motor1_IN2, 0);
    analogWrite(Motor2_IN3, 0);
    analogWrite(Motor2_IN4, turn_left);
    analogWrite(Motor3_IN5, 0);
    analogWrite(Motor3_IN6, turn_left);
    analogWrite(Motor4_IN7, turn_left);
    analogWrite(Motor4_IN8, 0);
    delay(10);
  } else if (PS4.RStickX() >= 40) {
    analogWrite(Motor1_IN1, turn_right);
    analogWrite(Motor1_IN1, 0);
    analogWrite(Motor1_IN2, turn_right);
    analogWrite(Motor2_IN3, turn_right);
    analogWrite(Motor2_IN4, 0);
    analogWrite(Motor3_IN5, turn_right);
    analogWrite(Motor3_IN6, 0);
    analogWrite(Motor4_IN7, 0);
    analogWrite(Motor4_IN8, turn_right);
    delay(10);
  } else if (Run == false) {
    analogWrite(Motor1_IN1, 0);
    analogWrite(Motor1_IN2, 0);
