...
//    Serial.println("Un.myservo1");
  ...
  fo = map(PS4.LStickY(), 60, 127, 55, 255);
  back = map(PS4.LStickY(), -127, -60, 255, 55);
  right = map(PS4.LStickX(), 40, 127, 55, 255);
  left = map(PS4.LStickX(), -127, -40, 255, 55);
  turn_right = map(PS4.RStickX(), 40, 127, 45, 255);
  turn_left = map(PS4.RStickX(), -127, -40, 255, 45);
